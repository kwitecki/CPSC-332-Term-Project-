...
if (isset($_GET['prof_ssn'])) {
  $stmt = $pdo->prepare("SELECT c.title, s.classroom, s.meeting_days, s.start_time, s.end_time FROM sections s JOIN courses c ON c.course_no = s.course_no WHERE s.professor_ssn = ? ORDER BY c.title, s.start_time");
...
if (isset($_GET['course_no'], $_GET['section_no'])) {
  $stmt = $pdo->prepare("SELECT grade, COUNT(*) AS cnt FROM enrollments WHERE course_no = ? AND section_no = ? GROUP BY grade ORDER BY FIELD(grade,'A','A-','B+','B','B-','C+','C','C-','D+','D','D-','F','W','I')");
...
<section>
  ...
  <form method="get">
    <label>Professor SSN: <input name="prof_ssn" placeholder="111223333" value="<?= htmlspecialchars($_GET['prof_ssn'] ?? '') ?>"></label>
...
<section>
  ...
  <form method="get">
    ...
    <label>Section No: <input name="section_no" placeholder="01" value="<?= htmlspecialchars($_GET['section_no'] ?? '') ?>"></label>
